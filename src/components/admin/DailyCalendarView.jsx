...
export default function DailyCalendarView({ appointments = [], onMove, movingAppointmentId = '' }) {
          ...
          {dayAppts.map((appt) => {
            ...
            return (
              <motion.div
                key={appt.id}
                layoutId={appt.id}
                className={`absolute right-14 left-2 rounded-xl border px-2 py-1.5 cursor-grab active:cursor-grabbing ${colorClass} ${movingAppointmentId === appt.id ? 'cursor-wait opacity-60' : ''}`}
                style={{ top, height }}
                whileHover={{ scale: 1.01 }}
                drag={movingAppointmentId ? false : 'y'}
                ...
                onDragEnd={(_, info) => {
                  ...
                  if (!movingAppointmentId && nextTime !== appt.time) onMove?.(appt, nextTime);
                }}
                aria-busy={movingAppointmentId === appt.id}
              >
